  ...
  try {
    await db.query(
      `INSERT INTO notification_logs (contract_id, client_email, channel, subject, status, sent_at, error_message)
       VALUES (?, ?, 'email', ?, ?, NOW(), ?)`,
      [fields.contractId, fields.to, fields.subject, fields.status, fields.errorMessage || null]
    );
  } catch (logErr) {
...
async function sendPaymentReminder(to, clientName, contractId, amountDue, dueDate, daysUntil) {
  ...
  try {
    ...
    await logNotification({ contractId, to, subject, status: 'failed', errorMessage: error.message });
    return { success: false, error: error.message };
  }

  // The email is already out — a logging problem past this point must not
  // flip the result back to failure.
  await logNotification({ contractId, to, subject, status: 'sent' });
